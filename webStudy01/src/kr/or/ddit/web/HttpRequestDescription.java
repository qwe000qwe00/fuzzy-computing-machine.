 ...
 *  GET : 조회 (R)
 *      Body 가 존재하지 않기 때문에, 전송할 최소한의 데이터를 Line의 URL에 덧붙임.
 *      ex) http://localhost/webStudy01/gugudan.do?minDan=4&maxDan=7     
 *      ?뒤 : 쿼리스트링 -> &로구분되는것:세그먼트(안에 파라미터와 파라미터값이 포함)
 ...
 * 							  useBodyEncodingForURI : POST 방식에서 사용하는 request.setCharacterEncoding(encoding)를 사용
