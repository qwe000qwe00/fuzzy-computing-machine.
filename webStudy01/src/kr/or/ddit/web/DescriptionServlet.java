 ...
 *   
 *   서블릿의 생명주기와 관련된 콜백 메소드의 종류 (호출되는 순서)
 *      init (initialize) : 서블릿의 객체가 생성된 직후에 호출되는 메소드
 *      service : 요청이 발생할 때마다 반복 호출
 *              요청의 HTTP method에 따라 doXXX계열 메소드를 호출
 *      doGet 등의 doXXX 계열 메소드
 *            : 요청이 발생할 때마다 반복 호출
 *              Http mehtod에 따라 service에 의해 호출됨 
 *      destroy : 서블릿의 객체가 소멸되기 직전에 호출되는 메소드 (소멸하는 시점이 명확하지 않음)
 *   
